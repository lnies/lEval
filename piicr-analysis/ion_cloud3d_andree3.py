...
def ion_cloud_3d(spot_positions, axis_scaling_factor, dont_show, file_name, number_of_clouds):
    ...
    if number_of_clouds == 1:
        ...
        if SIMION_file ==0:
            ...
            xss = [float(x[3])*axis_scaling_factor for x in spot_positions]  # in nanoseconds
            yss = [float(x[2])*axis_scaling_factor for x in spot_positions]  # in nanoseconds
        ...
        csv_files_2 = []
        for file in glob.glob("magnetic_field_spread_*.csv"):        # searches for all ...polar.csv files
        ...
        for i in csv_files_2:
            ...
            if SIMION_file ==0:
                ...
                xss = [float(x[3])*axis_scaling_factor for x in import_list_2]  # # in nanoseconds
                yss = [float(x[2])*axis_scaling_factor for x in import_list_2]  # in nanoseconds
...
if __name__ == '__main__':
    folder_path = 'G:\\Experiments\\ISOLTRAP\\USER\\Welker_Andree\\Magnetic field spread' # 'E:\\Welker\\Mag_Phase_Scan G:\\Experiments\\ISOLTRAP\\DATA\\2017\\2017-06-Cd-run\\129Cd_PIICR\\analyzed\\auto-run-004-jonas\\129Cs\\p1p2 G:\\Experiments\\ISOLTRAP\\USER\\Karthein_Jonas\\harm-new' or G:\\Experiments\\ISOLTRAP\\USER\\Karthein_Jonas\\Master\\harm-new\\test2
    os.chdir(folder_path)

    number_of_clouds = 1            # Please type the amount of to be collected .csv files here
